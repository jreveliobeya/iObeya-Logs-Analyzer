...
class TimelineCanvas(FigureCanvas):
    ...
    def _get_or_prepare_time_groups(self):
        ...
        dts = df['datetime_obj']
        if self.current_time_granularity == 'week':
            # Manually round to the beginning of the week (Monday) to avoid non-fixed frequency error
            rounded_time_series = dts.dt.normalize() - pd.to_timedelta(dts.dt.weekday, unit='d')
        elif self.current_time_granularity == 'day':
    ...
    def _do_delayed_plot_update(self):
        ...
        if self.hover_annotation:
            ...
            try:
                ...
                pass  # Artist may already be gone, which is fine.
            finally:
                self.hover_annotation = None
                self.last_hovered_bar_info = None
        ...
        if xlim_override is None:
            if x_pos.size > 0:
                ...
                self.full_time_min_num, self.full_time_max_num = None, None
                self.time_range_updated.emit(0, 0)

        bar_width = self._calculate_bar_width(times, x_pos, 0.7)

        temp_bars_data, _ = self._generate_timeline_bars(
            times, x_pos, time_groups, message_types_to_plot, bar_width
        )
        self.bars_render_data = list(reversed(temp_bars_data))

        self._configure_axes(xlim_override)
    ...
    def _calculate_bar_width(self, times, x_pos, bar_width_factor):
        ...
        if len(times) > 1:
            ...
            deltas = {'minute': timedelta(minutes=1), 'hour': timedelta(hours=1), 'day': timedelta(days=1), 'week': timedelta(weeks=1)}
    ...
    def _configure_axes(self, xlim_override):
        ...
        if self.current_time_granularity == 'week':
            ...
            major_formatter = mdates.DateFormatter('%m-%d %Hh')
        ...
        if self.current_time_granularity in ['hour', 'minute']:
    ...
    def on_hover(self, event):
        ...
        if hovered_bar_info:
            if hovered_bar_info == self.last_hovered_bar_info:
            ...
            self.last_hovered_bar_info = hovered_bar_info
            ...
            try:
                ...
                time_end = hovered_bar_info['time_end']
                te_display_obj = time_end - timedelta(microseconds=1)

                if time_start.date() == te_display_obj.date():
                    time_text = f"{time_start.strftime('%a %d/%m')} de {time_start.strftime('%H:%M:%S')} à {te_display_obj.strftime('%H:%M:%S')}"
                else:
                    ts_str = time_start.strftime('%a %d/%m %H:%M')
                    te_str = te_display_obj.strftime('%a %d/%m %H:%M')
                    time_text = f"De {ts_str} à {te_str}"

                text = f"{hovered_bar_info['message_type']}\n{time_text}\nTotal : {hovered_bar_info['count']}"
